...
namespace AlgoJudge.Server.Controllers
{
    ...
    ) : ControllerBase
    {
        ...
        /// <summary>
        /// The two redirect columns, as they are — which the public answer
        /// cannot say.
        /// <para>
        /// <b>Its own call rather than a field on <c>getInstanceInfo</c></b>,
        /// for two reasons rather than one. A visitor must never learn the slug
        /// of a provider that is switched off; and <see cref="AnnounceAsync"/>
        /// sends <c>InstanceInfoDto</c> to every user on the socket, so a field
        /// meant for a manager would reach everybody at the next instance write.
        /// </para>
        /// <para>
        /// Behind <c>instance:update</c>: the permission that writes these two
        /// columns, and the one the screen reading this already sits behind.
        /// </para>
        /// </summary>
        [HttpGet("redirects")]
        [ProducesResponseType<InstanceRedirectsDto>(StatusCodes.Status200OK)]
        public async Task<InstanceRedirectsDto> Redirects(CancellationToken ct)
        {
            await permissions.RequireAsync(Permissions.InstanceUpdate, null, ct);
            var instance = await instances.EnsureAsync(ct);

            var wanted = new[] { instance.SignInRedirectProvider, instance.RegisterRedirectProvider }
                .OfType<string>()
                .Distinct(StringComparer.Ordinal)
                .ToList();

            // Skipped rather than issued empty: `WHERE slug IN ()` is a round
            // trip for an answer already known.
            var named = wanted.Count == 0
                ? []
                : await context.IdentityProviders
                    .AsNoTracking()
                    .Where(p => wanted.Contains(p.Slug))
                    .Select(p => new { p.Slug, p.DisplayName, p.Enabled })
                    .ToListAsync(ct);

            InstanceRedirectDto Describe(string? slug)
            {
                if (slug is null)
                {
                    return new InstanceRedirectDto { State = "none" };
                }

                // Ordinal, which is the comparison `InstanceService.InForce`
                // makes against the same list. Two spellings of one predicate
                // that disagreed would put this screen back to lying, in the
                // other direction.
                var provider = named.FirstOrDefault(
                    p => string.Equals(p.Slug, slug, StringComparison.Ordinal));

                return new InstanceRedirectDto
                {
                    Slug = slug,
                    DisplayName = provider?.DisplayName,
                    State = provider is null ? "unregistered"
                        : provider.Enabled ? "inForce"
                        : "disabled",
                };
            }

            return new InstanceRedirectsDto
            {
                SignIn = Describe(instance.SignInRedirectProvider),
                Register = Describe(instance.RegisterRedirectProvider),
            };
        }
        ...
        /// <b>Aiming</b> a redirect at that is a mistake worth refusing at the
        /// moment it is made.
        /// </para>
        /// <para>
        ...
        /// provider is enabled — and that holds however the column was written.
        /// </para>
        /// </summary>
